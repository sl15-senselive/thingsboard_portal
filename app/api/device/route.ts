...
export async function POST(request: NextRequest) {
  try {
    ...
    const customerId = "f697f0f0-c771-11f0-b991-b7ab9ffff5c9";

    const deviceId = result.id.id;
    const created_at = result.createdTime;
    const date = new Date(created_at);
    const associateRes = await pool.query(
...
export async function GET() {
  try {
    ...
    const customerId = "f697f0f0-c771-11f0-b991-b7ab9ffff5c9";
    const res = await pool.query('SELECT * FROM device WHERE customer_id = $1', [customerId]);
